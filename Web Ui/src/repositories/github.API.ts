    ...
    constructor() {
      ...
      async obtainCommitsFromSha(owner: string, repoName: string, sha: string): Promise<CommitInformationDataObject[]> {
        try {
          ...
          const commits: CommitInformationDataObject[] = response.data.map((githubCommit: any) => {
            return{
              sha: githubCommit.sha,
              node_id: githubCommit.node_id,
              url: githubCommit.url,
              ...
              stats: {
                total: githubCommit.stats.total.toString(),
                additions: githubCommit.stats.additions.toString(),
                deletions: githubCommit.stats.deletions.toString(),
              },
              files: githubCommit.files.map((file: any) => {
                return {
                  sha: file.sha,
