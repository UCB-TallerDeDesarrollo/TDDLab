...
export function usePracticeDetail({
  ...
  const [practice, setPractice] =
    useState<PracticeDataObject | null>(null);

  const [submission, setSubmission] =
    useState<PracticeSubmissionDataObject | null>(null);

  const [practiceSubmissions, setPracticeSubmissions] =
  ...
  const sendComment = async (
    ...
    await finishPracticeSubmission(
      submission.id,
      data
    );
