...
export default function LoginComponent({ loginModalOpen, setLoginModalOpen }: LoginComponentProps) {
  ...
  // ESTADOS PARA LOS POP-UPS DE VALIDACIÓN (Error/Éxito)
  ...
  const showDialog = (message: string, error = true) => {
    setLoginModalOpen(false); // Cerramos el de selección para que no se encimen
  ...
  // ESTA ES LA FUNCIÓN QUE MANEJA AMBOS PROVEEDORES
  const handleLoginAction = async (provider: 'github' | 'google') => {
    try {
  ...
  return (
    <div className="user-profile-group">
      {authData[0].userEmail ? (
        <>
          <Avatar src={authData[0].userProfilePic} alt="Profile" onClick={handleClick} className="user-avatar" style={{ cursor: "pointer" }} />
          <Menu anchorEl={anchorEl} open={open} onClose={handleClose} transformOrigin={{ horizontal: 'right', vertical: 'top' }} anchorOrigin={{ horizontal: 'right', vertical: 'bottom' }}>
            <MenuItem onClick={handleGoToProfile} className="user-menu-item">
              <ListItemIcon><AccountCircle fontSize="small" /></ListItemIcon> Mi Perfil
            </MenuItem>
            <Divider />
            <MenuItem onClick={handleLogoutAction} className="logout-menu-item">
              <ListItemIcon><Logout className="logout-icon-red" /></ListItemIcon> Cerrar Sesión
            </MenuItem>
          </Menu>
        </>
      ) : (
        /* IMPORTANTE: Este botón ahora solo ABRE el modal */
        <Button onClick={() => setLoginModalOpen(true)} className="btn-std btn-primary">
          Ir a TDD Lab
        </Button>
      )}

      {/* POP-UP DE SELECCIÓN DE CUENTA (MUESTRA AMBOS) */}
      <Dialog open={loginModalOpen} onClose={() => setLoginModalOpen(false)} maxWidth="xs" fullWidth>
        <Box p={3} textAlign="center">
          ...
          <Button 
            ...
            fullWidth 
            onClick={() => handleLoginAction('github')} 
          ...
          <Button 
            ...
            fullWidth 
            onClick={() => handleLoginAction('google')} 
