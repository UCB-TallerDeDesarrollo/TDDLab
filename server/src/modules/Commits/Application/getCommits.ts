...
export class CommitUseCases {
  ...
  constructor(
    ...
    this.commitTableUseCases = new CommitTableUseCases(this.repositoryAdapter, this.githubAdapter);
  }

  async getCommits(owner: string, repoName: string) {
    try {
      if (!await this.repositoryAdapter.repositoryExist(owner, repoName)) {
        const commits = await this.commitTableUseCases.getCommitsAPI(owner, repoName);
        this.commitTableUseCases.saveCommitsDB(owner, repoName, commits);
      } else {
        const commits = await this.commitTableUseCases.getCommitsAPI(owner, repoName); //getCommitsAPI should be changed to getLastCommits once it is implemented
        const newCommits = await this.commitTableUseCases.checkNewCommits(owner, repoName, commits);
