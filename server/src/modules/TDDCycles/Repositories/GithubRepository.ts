...
// import { JobDataObject } from "../Domain/JobDataObject"; // Eliminado visualmente
...
export class GithubRepository implements IGithubRepository {
  ...
  async getCommits(owner: string, repoName: string): Promise<CommitDataObject[]> {
    try {
  ...
  async getCommitInfoForTDDCycle(
    ...
    try {
      ...
      if (coverageResponse.data.length > 0) {
        ...
        if (coverageMatch) percentageMatch = String(coverageMatch[1]);
        if (testCountMatch) testCount = String(testCountMatch[1]);
      }
  ...
  async fetchCoverageDataForCommit(owner: string, repoName: string, sha: string) {
    try {
      ...
      if (coverageResponse.data.length > 0) {
        ...
        if (coverageMatch) coveragePercentage = coverageMatch[1];
        else console.warn("No se encontró el patrón de cobertura en el body.");
      } else {
  ...
  // Funciones comentadas para "ocultar" lógica de jobs sin borrarlas
  /*
  ...
  async getRunsOfGithubActionsIds(owner: string, repoName: string) {
    ...
    return githubruns.data.workflow_runs.map((workFlowRun: any) => [
      workFlowRun.head_commit.id,
      workFlowRun.id,
    ]);
  }
  ...
  async getJobsDataFromGithub(
    ...
    await Promise.all(
      listOfCommitsWithActions.map(async ([sha, runId]) => {
        const jobInfo = await this.obtainJobsOfACommit(owner, repoName, runId, 1);
        jobs[sha] = jobInfo;
      })
    );
    return jobs;
