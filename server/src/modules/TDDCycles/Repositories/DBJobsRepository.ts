...
export class DBJobsRepository implements IDBJobsRepository {
  ...
  async findJobByCommit(sha: string, owner: string, repoName: string): Promise<any | null> {
    ...
    try {
      ...
      console.error("Error buscando el commit en jobsTable:", error);
      throw error;
    } finally {
      client.release();
    }
  ...
  // ========= MÉTODOS LEGADOS (JOBS) OCULTOS =========

  // Método legado (oculto): actualizar conclusión del job
  private async updateJobConclusion_legacy(sha: string, repoOwner: string, repoName: string, conclusion: string): Promise<void> {
    const client = await this.pool.connect();
    try {
      const query = `
        UPDATE jobstable
  ...
  // Método legado (oculto): guardar job desde log
  private async saveJobFromTDDLog_legacy(job: TestResultDataObject): Promise<void> {
    const client = await this.pool.connect();
    try {
      const query = `
  ...
  // Método legado (oculto): guardar un job completo
  private async saveJob_legacy(job: TestResultDataObject) {
    const client = await this.pool.connect();
    try {
  ...
  // Método legado (oculto): obtener todos los jobs de un repo
  private async getJobs_legacy(owner: string, repo: string) {
    let client;
    try {
  ...
  // Método legado (oculto): comprobar si existe un job por ID
  private async jobExists_legacy(owner: string, repo: string, jobId: number) {
    let client;
    try {
      client = await this.pool.connect();
      const query =
  ...
  // Método legado (oculto): comprobar si existe repositorio
  private async repositoryExists_legacy(owner: string, repoName: string): Promise<boolean> {
    const client = await this.pool.connect();
    try {
      const query =
  ...
  // Método legado (oculto): commits aún no guardados
  private async getJobsNotSaved_legacy(
    owner: string,
    repoName: string,
    ...
    for (const currentJob of commitsWithActions) {
      let exists = await this.jobExists_legacy(owner, repoName, currentJob[1]);
      if (exists) break;
  ...
  // Método legado (oculto): guardar varios jobs
  private async saveJobsList_legacy(
    owner: string,
    repoName: string,
    ...
    await Promise.all(jobsFormatted.map((job) => this.saveJob_legacy(job)));
  }
