...
export class FirebaseDBBranchesCommitsRepository implements IFirebaseDBBranchesCommitsRepository {
    ...
    try {
      ...
      const branchesWithCommits = await Promise.all(
        snapshot.docs.map(async (doc) => {
          ...
          let commits: IDBCommit[] = await this.getCommitsFromShas(branchCommitsShas);
          
          if (commits.length === 0) {
            commits = await this.getCommitsByBranchInfo(owner, repoName, branchName);
          }

          commits = await this.attachTestRunsToCommits(commits);
          
          const updatedAt = this.parseDate(branchData.updated_at || branchData[" updated_at"]);
  ...
  private async getCommitsFromShas(branchCommitsShas: string[]): Promise<IDBCommit[]> {
    const commits: IDBCommit[] = [];
  ...
  private async getCommitsByBranchInfo(owner: string, repoName: string, branchName: string): Promise<IDBCommit[]> {
    const commitsRef = db.collection("commits");
    ...
    if (!commitsSnapshot.empty) {
        return commitsSnapshot.docs.map(doc => {
            const data = doc.data();
            return { 
                ...data, 
  ...
  private async attachTestRunsToCommits(commits: IDBCommit[]): Promise<IDBCommit[]> {
    if (commits.length > 0) {
  ...
  private parseDate(updatedAt: any): Date | any {
