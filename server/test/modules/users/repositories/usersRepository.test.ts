...
describe("getUsersByGroupId", () => {
  it("should retrieve users by group ID", async () => {
    ...
    expect(result).toEqual(expectedUsers.map(user => repository.mapRowToUser(user)));
...
describe('registerUser', () => {
  it('should insert a new user into the database', async () => {
    ...
    expect(clientQueryMock).toHaveBeenCalledWith(
      'INSERT INTO usersTable (email,groupid,role,first_name,last_name) VALUES ($1, $2, $3, $4, $5)',
      [newUser.email, newUser.groupid, newUser.role, newUser.firstName, newUser.lastName]
    );
  });
...
describe('obtainUsers', () => {
  it('should return all users when found', async () => {
    ...
    expect(clientQueryMock).toHaveBeenCalledWith(
      'SELECT id, email, groupid, role, first_name, last_name FROM usersTable', 
      undefined
    );
  });
...
describe("obtainUserById", () => {
  it("should return a user when found by id", async () => {
    ...
    expect(user).toEqual(repository.mapRowToUser(expectedUser));
    expect(clientQueryMock).toHaveBeenCalledWith(
      "SELECT id, email, groupid, role, first_name, last_name FROM usersTable WHERE id = $1",
      [id]
    );
  });
...
describe("obtainUserByEmail", () => {
  it("should return a user when found by email", async () => {
    ...
    expect(clientQueryMock).toHaveBeenCalledWith(
      "SELECT id, email, groupid, role, first_name, last_name FROM usersTable WHERE email = $1",
      [email]
    );
  });
...
describe("updateUserGroup", () => {
  it("should update the user's group and return the updated user", async () => {
    ...
    expect(result).toEqual(repository.mapRowToUser(updatedUser));
    expect(clientQueryMock).toHaveBeenCalledWith(
